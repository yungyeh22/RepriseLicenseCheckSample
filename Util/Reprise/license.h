...
#define RLM_VERSION 14 
#define RLM_REVISION 0
#define RLM_BUILD 2
#define RLM_PATCH 0

#define RLM_SOFTWARE_VERSION  "Reprise License Manager (RLM) v14.0, Copyright (C) 2006-2019, Reprise Software, Inc."
#define RLM_COPYRIGHT  "Copyright (C) 2006-2019, Reprise Software, Inc. All rights reserved."
...
#define RLM_ACT_HANDLE_CONTACT  10	
#define RLM_ACT_HANDLE_COMPANY	11
#define RLM_ACT_HANDLE_EMAIL	12	
#define RLM_ACT_HANDLE_STATE	13
#define RLM_ACT_HANDLE_COUNTRY	14
#define RLM_ACT_HANDLE_U1	15
#define RLM_ACT_HANDLE_U2	16

/*
 *	Act handle max sizes
 */

#define RLM_ACT_MAX_CONTACT	60
#define RLM_ACT_MAX_COMPANY	60
#define RLM_ACT_MAX_EMAIL	60
#define RLM_ACT_MAX_STATE	20
#define RLM_ACT_MAX_COUNTRY	60
#define RLM_ACT_MAX_U1		60
#define RLM_ACT_MAX_U2		60
/*
...
/*
 *	Hostid types to disable
 */
#define RLM_DISABLE_H_32BIT  1
#define RLM_DISABLE_H_STRING 2		/* Equivalent to ANY, works anywhere */
#define RLM_DISABLE_H_ETHER  4
#define RLM_DISABLE_H_USER   8		/* User= */
#define RLM_DISABLE_H_HOST  0x10 	/* Host= */
#define RLM_DISABLE_H_IP    0x20 	/* IP address */
#define RLM_DISABLE_H_ANY   0x40	/* ANY - valid anywhere */
#define RLM_DISABLE_H_DEMO  0x80	/* DEMO - valid anywhere */
#define RLM_DISABLE_H_SN    0x100	/* Serial # - just like STRING */
#define RLM_DISABLE_H_RLMID1 0x200	/* First dongle type (aladdin) */
#define RLM_DISABLE_H_RLMID2 0x400	/* Second dongle type (safenet) */
#define RLM_DISABLE_H_DISKSN 0x800	
#define RLM_DISABLE_H_GC     0x1000	/* Google Cloud hostid (v11.1) */
#define RLM_DISABLE_H_IPV6   0x2000	/* IPv6 address */
#define RLM_DISABLE_H_UUID   0x4000	/* BIOS UUID (Windows only) */
...
extern int			rlm_add_isv_hostid_multiple(RLM_HANDLE, 
						...
						int (*)(char *, RLM_HANDLE), 
						int (*)(char *, RLM_HANDLE));
extern int			rlm_add_meter_count(RLM_HANDLE, RLM_PRODUCTS, 
								int, int);
extern char **			rlm_all_hostids(RLM_HANDLE, int);
extern void			rlm_all_hostids_free(char **);
extern int			rlm_auth_check(RLM_LICENSE, const char *);
...
extern int			rlm_get_attr_lic_check(RLM_LICENSE, 
						char **license, char **hostid);
extern void			rlm_get_proxy(const char *url, char *proxy);
extern int			rlm_get_rehost(RLM_HANDLE, const char *, char *);
...
extern void			rlm_isv_cfg_disable_generic_server(
							    RLM_HANDLE, int);
extern void			rlm_isv_cfg_disable_hostids(RLM_HANDLE, int);
...
extern void			rlm_isv_cfg_enable_roam_extend(RLM_HANDLE, int);
extern void			rlm_isv_cfg_no_server_license_fail(RLM_HANDLE, 
									int);
...
extern void			rlm_isv_cfg_set_isv_handshake(RLM_HANDLE, int,
									int);
extern void			rlm_isv_cfg_set_http_promise(RLM_HANDLE, int);
extern void			rlm_isv_cfg_set_http_promise(RLM_HANDLE, int);
...
extern void			rlm_isv_cfg_set_oldest_server(RLM_HANDLE, 
							...
							int oldest_rev,
							int oldest_build);
extern void			rlm_isv_cfg_set_rehost_dir(RLM_HANDLE, 
							const char *);
extern void			rlm_isv_cfg_set_roam_single(RLM_HANDLE, int);
extern void			rlm_isv_cfg_set_server_roam(RLM_HANDLE, 
							const char *);
extern void			rlm_isv_cfg_set_url(RLM_HANDLE, const char *);
...
extern int			rlm_sign_license(RLM_HANDLE rh, int encode_bits,
					  const char *server_hostid, char *license);
...
extern int 		rlm_license_ismetered(RLM_LICENSE lic);
...
extern int		rlm_license_meter_counter(RLM_LICENSE lic);
 ...
 *	rlm_products() data
 */
extern RLM_PRODUCTS 	rlm_products(RLM_HANDLE, const char *, const char *);
extern RLM_PRODUCTS	rlm_products_dynres(RLM_HANDLE rh, const char *product, 
					const char *ver, const char *dynres);
...
extern int 		rlm_product_ismetered(RLM_PRODUCTS);
extern char *		rlm_product_issuer(RLM_PRODUCTS);
extern int		rlm_product_meter_counter(RLM_PRODUCTS);
extern int		rlm_product_meter_cur_count(RLM_PRODUCTS);
...
extern int		rlm_product_named_user_count(RLM_PRODUCTS);
...
extern char *		rlm_product_start(RLM_PRODUCTS);
...
extern void		rlm_set_attr_password(RLM_HANDLE, const char *);
extern void		rlm_set_attr_reference_hostid(RLM_HANDLE, const char *);
extern void		rlm_set_attr_req_opt(RLM_HANDLE, const char *);
extern void		rlm_set_auto_hb_isvdata(RLM_HANDLE, void *i);
extern void		rlm_set_environ(RLM_HANDLE, const char *, const char *, 
						const char *);
...
extern int		rlm_act_fulfill_info(RLM_HANDLE rh, const char *url, 
					const char *akey, char *product, char *ver, 
...
extern int		rlm_act_info(RLM_HANDLE rh, const char *url, 
					const char *akey, char *product, 
					char *ver, int *date_based, 
					int *license_type, char *upgrade_version);
extern int		rlm_act_keyinfo(RLM_HANDLE rh, const char *url, 
					...
					int *fulfilled, int *rehosts, 
					int *revoked);
extern int		rlm_act_keyinfo2(RLM_HANDLE rh, const char *url, 
					const char *akey, char *product, char *ver, 
					int *date_based, int *license_type,
					char *upgrade_version, int *count,
					int *fulfilled, int *rehosts, 
					int *revoked, char *exp, char *hostid,
					char *keyexp, int *allowed_hostids,
					int *sub_interval, int *sub_window);
extern int		rlm_act_keyvalid(RLM_HANDLE rh, const char *url,
					const char *akey, char *hostid);
extern int		rlm_act_keyvalid_license(RLM_HANDLE rh, 
					const char *url, const char *akey, 
					char *hostid, char *license);
...
/*
 *	Bad returned license but client has set "nofail"
 *	These status returns are from rlm_get_attr_lic_check()
 */

#define RLM_NO_SERVER_LIC    1		/* No license returned from server */
#define RLM_LIC_BAD	     2		/* License doesn't verify */
...
#define RLM_EH_BADLOGIN		-182	/* Bad/No login credentials to server */
#define RLM_EH_WS_NOSUPP	-183	/* Function not supported with web 
								services */
#define RLM_EH_NOFUNC		-184	/* Function not available */

#define RLM_EH_MAX_ERRS 83
...
#define RLM_EL_NESTED_ALIAS	-67	/* Nested aliases */
#define RLM_EL_NO_JSON	 	-68	/* No JSON in returned message */
#define RLM_EL_BAD_JSON	 	-69	/* Bad JSON in returned message */
#define RLM_EL_BADHANDSHAKE 	-70	/* Bad handshake on web svc checkout */
#define RLM_EL_HELPER_ERR	-71	/* rlm_helper error */

#define RLM_EL_MAX_ERRS 71
...
#define RLM_ACT_CONTACT_BAD	-1044 /* Contact information supplied is bad */
#define RLM_ACT_ERR_LAST	44
